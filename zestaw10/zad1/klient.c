...
{ 
    ...
    msg.msgType = STOP;
    msg.clientID = myID;
    write(mySocket, (char *)&msg, MSG_SIZE);
    exit(0); 
}
...
int main(int argc, char ** argv)
{
    ...
    struct msg msg;
    msg.msgType = INIT;
    write(mySocket, &msg, MSG_SIZE);

    struct msg currentMsg;

    while (7)
    {
        int t = read(mySocket, &currentMsg, MSG_SIZE);
    ...
    while (7)
    {
        t = recv(mySocket, (char *)&currentMsg, MSG_SIZE, MSG_DONTWAIT);
        if(t != -1)
        {
