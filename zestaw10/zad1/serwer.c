...
void handler()
{
    struct msg msg;
    msg.msgType = STOP;
    for(int i = 0; i < MAX_CLIENTS; i++) if(clientSockets[i] != -1) 
    {
        write(clientSockets[i], (char *)&msg, MSG_SIZE);
...
void pinging()
{
    ...
    while (7)
    {
        ...
        {
            activeClients[i] = -1;
            write(clientSockets[i], (char *)&pingingMsg, MSG_SIZE);
        }
        pthread_mutex_unlock(&ping_mutex);
        sleep(7);
        pthread_mutex_lock(&ping_mutex);
        for (int i = 0; i < MAX_CLIENTS; i++) if (clientSockets[i] != -1 && activeClients[i] == -1) 
        {
            close(clientSockets[i]);
            clientSockets[i] = -1;  
        } 
...
int check_for_message(struct msg * currentMsg)
{
    ...
    while (7)
    {
        ...
        acceptingSocket = accept(mySocket, (struct sockaddr *)&clientArguments, (socklen_t *)&size);
        if (acceptingSocket != -1) return read(acceptingSocket, (char *)currentMsg, MSG_SIZE);

        for (int i = 0; i < MAX_CLIENTS; i++) if (clientSockets[i] != -1)
        {
            t = recv(clientSockets[i], (char *)currentMsg, MSG_SIZE, MSG_DONTWAIT);
            if (t != -1) return t;
        }
    } 
}
...
void changeMode(struct msg * currentMsg)
{
    ...
    if (type == INIT)
    {
        ...
        pthread_mutex_lock(&ping_mutex);
        clientSockets[newClientID] = acceptingSocket;
        ...
        write(clientSockets[newClientID], (char *)&msg, MSG_SIZE);
        newClientID++;
    ...
    else if(type == LIST)
    {
        ...
        write(clientSockets[currentMsg->clientID], (char *)&msg, MSG_SIZE);
    }
    else if(type == TOALL)
    {
        struct msg msg;
        ...
        for(int i = 0; i < MAX_CLIENTS; i++) if(clientSockets[i] != -1) write(clientSockets[i], (char *)&msg, MSG_SIZE);
    }
    else if(type == TOONE)
    {
        struct msg msg;
        if(clientSockets[currentMsg->toOne] == -1) return;
        msg.clientID = currentMsg->clientID;
        msg.msgType = PRINT;
        strcpy(msg.text, currentMsg->text);
        strcpy(msg.date, asctime(timeInfo));
        write(clientSockets[currentMsg->toOne], (char *)&msg, MSG_SIZE);
    }
    else if(type == STOP)
    {
        pthread_mutex_lock(&ping_mutex);
        close(clientSockets[currentMsg->clientID]);
        clientSockets[currentMsg->clientID] = -1;
        pthread_mutex_unlock(&ping_mutex);
    } 
    else if(type == PING)
    {
